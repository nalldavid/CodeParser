...
//    fprintf(parseLog,"Parsing %s.\n",n);
    fprintf(parseLog,"Parsing %s.\n",n);
    if (stat(n, &stbuf) == -1) {
           fprintf(parseLog, "callParse: can't access %s\n", n);
           return;
    }
    if ((stbuf.st_mode & S_IFMT) == S_IFDIR){dirwalk(name, callParse);}
    else{parse(name);}
    //fprintf(parseLog,"%8ld %s\n", stbuf.st_size, name);
}
...
void *parse(char * name){
  ...
  for(j=0;j<MAX_PATH;j++){
       if(*name==0){
           if(j==(MAX_PATH-2)){fprintf(parseLog,"Path too long.\n");return;}
           name--;
           if(*name=='c'){
               name--;
               if(*name=='.'){
  ...
  char c;
  char std[9]={'<','s','t','d','i','o','.','h','>'};
  char stdWhole[19]={'#','i','n','c','l','u','d','e',' ','<','s','t','d','i','o','.','h','>','\n'};
  ...
  do{
      c=fgetc(readFile);
      hold[8]=c;
      for(j=0;j<9;j++){if(hold[j]!=std[j]){break;}else{if(j==8){stdIncluded=true;}}}
      if(stdIncluded){break;}
      for(j=0;j<9;j++){hold[j]=hold[j+1];}
  }while(c!=EOF);
  fclose(readFile);
  readFile=fopen(nameString,"r");
  ...
  if(!stdIncluded){
      for(j=0;j<19;j++){fputc(stdWhole[j],writeFile);}
  }
